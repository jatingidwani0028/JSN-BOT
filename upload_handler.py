...
async def cmd_upload(message: Message, state: FSMContext) -> None:
    ...
    if len(parts) < 2:
        await message.answer("⚠️ Usage: /upload folder_name\nThen send your .json file.")
        return
    ...
    await message.answer(f"📤 Ready to upload to '{folder_name}'.\nSend your .json file now.")
...
async def cb_upload_action(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(UploadState.waiting_for_file)
    await state.update_data(folder_name=folder_name)
    await callback.message.answer(f"📤 Ready to upload to '{folder_name}'.\nSend your .json file now.")
...
async def handle_json_upload(message: Message, state: FSMContext) -> None:
    ...
    if doc.file_size > MAX_JSON_FILE_SIZE:
        await message.answer(f"❌ File too large ({doc.file_size // 1024} KB). Max: {MAX_JSON_FILE_SIZE // 1024} KB.")
        return
    ...
    await message.answer("📤 Send another .json file or /cancel to stop.")
